...
END
+ ~IsValidForPartyDialog("Minsc")~ + ~First a space hamster and now a metal squirrel. Just wonderful.~ EXTERN %MINSC_JOINED% t2.minsc
++ ~This... squirrel's your pet, I take it?~ + t2.5
++ ~I don't care. Just pack him away so we can keep moving.~ + t2.0

CHAIN %MINSC_JOINED% t2.minsc
~Boo thinks he'd like your Muffy, Aura.~
== C0AURAJ ~You sure? Muffy's a little bit mean to anything smaller than him.~
== %MINSC_JOINED% ~Fear not, for while Boo may be small, his heart is larger than... no, Boo! Stop fighting with the nice squirrel!~ [GAM_48]
...
== %IMOEN_JOINED% IF ~InParty("%IMOEN_DV%")~ THEN ~Umm, can anyone explain to me what in Oghma's name these two are talking about?~
== %QUAYLE_JOINED% IF ~InParty("Quayle")~ THEN ~Hah! As if my mighty brain could not understand every word they're saying. Uh... they *are* talking about the fungus that's growing here, right?~
== %VICONIA_JOINED% IF ~InParty("Viconia")~ THEN ~This may be a rare sentence for me, but I implore you, kivvil, speak Common.~
== %KAGAIN_JOINED% IF ~InParty("Kagain")~ THEN ~I don't give two damns of whatever you louts are sayin', but you'd best answer the most important question: is there any gold to be found in this stink-hole that makes this worth my time?~
== %MINSC_JOINED% IF ~InParty("Minsc")~ THEN ~Boo? Would you explain to Minsc what Aura and the dusty man are saying? ...*squeak* Ah! Now Minsc understands completely! You are wise as ever, my friend!~
END
...
IF ~GlobalLT("Chapter","GLOBAL",%tutu_chapter_5%) Global("C0AuraLeaves","GLOBAL",1)~ angry
